...
type ChatMessage = { body: string; from: "contact" | "ai" | "human"; time: string; status?: "PENDING" | "SENT" | "DELIVERED" | "READ" | "FAILED" };
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {view === "Conversaciones" && (active ? <Inbox conversations={conversations} setConversations={setConversations} active={active} setActiveId={setActiveId} notify={notify} onAppointment={() => setModal(true)} userName={userName} /> : <EmptyState title="Sin conversaciones todavía" note="Cuando conectes canales reales o cargues conversaciones, aparecerán aquí. No se muestran datos inventados en cuentas nuevas." />)}
...
function PublicSite({ onLogin, onRegister }: { onLogin: (email:string,password:string,remember:boolean)=>Promise<void>; onRegister: (data:{name:string;email:string;password:string;businessName:string;plan:BillingPlan;interval:BillingInterval;startWithTrial:boolean})=>Promise<void> }) {
  ...
  return <div className="public-site"><header className="public-nav"><div className="brand"><span className="brand-mark">n</span><span>next.io <span>by Mercadia</span></span></div><nav><button onClick={()=>scrollTo("inicio")}>Inicio</button><button onClick={()=>scrollTo("servicios")}>Servicios</button><button onClick={()=>scrollTo("precios")}>Precios</button><button className="primary" onClick={()=>setPage("login")}>Iniciar sesion</button></nav></header><main className="public-content"><section id="inicio" className="public-hero"><div className="public-copy"><span className="eyebrow">SAAS OMNICANAL CON IA</span><h1>Responde mas rapido, ordena tus canales y convierte conversaciones en clientes.</h1><p>next.io by Mercadia centraliza Instagram, WhatsApp Business, Facebook, contactos, conocimiento del negocio, equipo, planes y pagos en un solo workspace.</p><div className="hero-actions"><button className="primary" onClick={()=>setPage("login")}>Crear cuenta</button><button onClick={()=>scrollTo("servicios")}>Ver servicios</button></div></div><div className="public-visual"><div className="visual-window"><div><span/><span/><span/></div><strong>Inbox + IA + Equipo</strong><p>Centraliza la atencion y usa solo el conocimiento que configures para cada negocio.</p><section><b>IG</b><b>WA</b><b>AI</b></section></div></div></section><section id="servicios" className="public-section"><span className="eyebrow">SERVICIOS</span><h2>Una operacion comercial ordenada desde un solo lugar.</h2><div className="public-grid"><article className="card"><h3>Atencion omnicanal</h3><p>Organiza contactos y conversaciones de Instagram, WhatsApp Business y Facebook conforme conectes cada canal.</p></article><article className="card"><h3>IA entrenable</h3><p>Configura reglas, productos, servicios, horarios y politicas para responder con informacion aprobada por tu negocio.</p></article><article className="card"><h3>Control de consumo</h3><p>Los planes limitan conversaciones, respuestas IA, usuarios y canales para que el cobro sea claro.</p></article></div></section><section id="precios" className="public-section"><span className="eyebrow">PRECIOS</span><h2>Elige el volumen que necesita tu negocio.</h2><div className="pricing-grid public-pricing"><article className="pricing-card"><span>Micro</span><h3>$49 MXN/mes</h3><p>20 conversaciones, 20 respuestas IA, 1 usuario y 1 canal. Sin prueba gratuita: se activa al pagar.</p><button className="primary" onClick={()=>setPage("login")}>Elegir Micro</button></article><article className="pricing-card"><span>Starter</span><h3>$999 MXN/mes</h3><p>500 conversaciones/mes para equipos que empiezan con inbox y conocimiento.</p><button className="primary" onClick={()=>setPage("login")}>Elegir Starter</button></article><article className="pricing-card current"><span>Growth</span><h3>$1,999 MXN/mes</h3><p>2,000 conversaciones/mes para negocios con mayor volumen y equipo comercial.</p><button className="primary" onClick={()=>setPage("login")}>Elegir Growth</button></article><article className="pricing-card"><span>Advanced</span><h3>$3,499 MXN/mes</h3><p>5,000 conversaciones/mes para operaciones con limites mas altos y mas miembros.</p><button className="primary" onClick={()=>setPage("login")}>Elegir Advanced</button></article></div></section></main><footer className="public-footer"><div><div className="brand"><span className="brand-mark">n</span><span>next.io <span>by Mercadia</span></span></div><p>Operacion omnicanal con IA para negocios en Mexico.</p></div><nav aria-label="Informacion legal"><a href="/terminos">Terminos y condiciones</a><a href="/privacidad">Aviso de privacidad</a><a href="/eliminacion-de-datos">Eliminacion de datos</a></nav></footer></div>;
}

function Login({ onLogin, onRegister }: { onLogin: (email:string,password:string,remember:boolean) => Promise<void>; onRegister: (data:{name:string;email:string;password:string;businessName:string;plan:BillingPlan;interval:BillingInterval;startWithTrial:boolean}) => Promise<void> }) {
  ...
  const submit = async(e: FormEvent) => {
    ...
    setError("");
    try {
      mode==="login" ? await onLogin(email,password,remember) : await onRegister({name,email,password,businessName,plan,interval,startWithTrial:plan === "MICRO" ? false : startWithTrial});
      if(mode==="login"){
  ...
  return <div className="login-page">
    ...
    <div className="login-panel"><form onSubmit={submit} autoComplete={mode==="login"?"on":"off"}>
      ...
      <small className="legal">Al continuar, aceptas los <a href="/terminos">Terminos y condiciones</a> y el <a href="/privacidad">Aviso de privacidad</a>.</small>
    </form></div>
  </div>;
}
...
function Inbox({ conversations, setConversations, active, setActiveId, notify, onAppointment, userName }: { conversations: Conversation[]; setConversations: (v: Conversation[]) => void; active: Conversation; setActiveId: (id:string | number)=>void; notify:(s:string)=>void; onAppointment:()=>void; userName:string }) {
  ...
  const send = async () => {
    ...
    if (typeof active.id === "string") {
      try {
        const message = await api.sendMessage(active.id, body);
        syncConversation(await api.conversation(active.id));
        notify(message.status === "FAILED" ? "No se pudo enviar por el canal. Revisa la conexión de Meta." : "Mensaje enviado por el canal.");
        return;
      } catch {
  ...
  return <div className="inbox"><section className="thread-list"><div className="inbox-title"><div><h2>Conversaciones</h2><span>{conversations.reduce((sum,item)=>sum+item.unread,0)} sin leer</span></div><button onClick={()=>notify("Nueva conversación local creada") } aria-label="Nueva conversación">+</button></div><div className="search"><span>⌕</span><input aria-label="Buscar conversaciones" placeholder="Buscar conversaciones..." value={query} onChange={e=>setQuery(e.target.value)} /></div><div className="filters">{["Todos","Instagram","WhatsApp","Facebook","Frío","Tibio","Caliente","IA activa","Pausadas","Humano","Con cita"].map(f=><button className={filter===f?"active":""} onClick={()=>setFilter(f)} key={f}>{f}</button>)}</div><div className="threads">{filtered.length ? filtered.map(c=><button key={c.id} className={active.id===c.id?"active":""} onClick={()=>{setActiveId(c.id);setNoteDraft(c.note)}}><span className="avatar" style={{background:c.color}}>{c.initials}</span><span className="thread-copy"><b>{c.name}<i className={`channel ${c.channel}`}>{CHANNELS[c.channel].short}</i></b><small>{c.last}</small><em className={`temp ${c.temperature.toLowerCase()}`}>● {c.temperature}</em>{c.ai ? <em className="ai-label">✦ IA activa</em>:<em className="ai-label paused">Ⅱ IA pausada</em>}{c.transferred&&<em className="ai-label">Humano</em>}</span><span className="thread-meta"><time>{c.time}</time>{c.unread>0&&<b>{c.unread}</b>}</span></button>) : <div className="empty"><b>Sin resultados</b><span>Prueba con otro filtro.</span></div>}</div></section>
    <section className="chat"><div className="chat-head"><span className="avatar" style={{background:active.color}}>{active.initials}</span><div><b>{active.name}</b><span><i className="online" /> En línea · {CHANNELS[active.channel].label}</span></div><div className="chat-actions"><button onClick={active.ai ? take : returnToAi}>✦ {active.ai?"Pausar IA":"Activar IA"}</button><button className={active.transferred?"active":""} onClick={active.transferred ? returnToAi : take}>⇄ {active.transferred?"Devolver a IA":"Tomar conversación"}</button><button aria-label="Más opciones">•••</button></div></div><div className="chat-note"><span>✦</span><p><b>Resumen de IA</b> {active.summary}</p></div><div className="messages"><div className="date-chip">HOY</div>{active.messages.map((message,i)=><div className={`bubble-row ${message.from === "contact" ? "in" : "out"}`} key={`${active.id}-${i}`}>{message.from === "contact"&&<span className="avatar tiny" style={{background:active.color}}>{active.initials}</span>}<div className="bubble">{message.from === "ai"&&<small>✦ Nia · IA</small>}{message.from === "human"&&<small>{userName} · Agente</small>}<p>{message.body}</p><time>{message.time} {message.from !== "contact" ? (message.status === "FAILED" ? "No enviado" : message.status === "PENDING" ? "Enviando…" : "✓✓") : ""}</time></div></div>)}</div><div className="quick-replies"><button onClick={replyWithAi} disabled={aiLoading}>{aiLoading?"Generando IA...":"Responder con IA"}</button><button onClick={()=>setDraft("¿Te gustaría agendar una videollamada de 20 minutos?")}>Agendar videollamada</button><button onClick={()=>setDraft("Te comparto nuestros planes disponibles.")}>Compartir planes</button><button onClick={()=>setDraft("¿Hay algo más en lo que pueda ayudarte?")}>Cerrar conversación</button></div><div className="composer"><button aria-label="Adjuntar">+</button><textarea aria-label="Mensaje" value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Escribe un mensaje..." onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}}/><button aria-label="Emoji">☺</button><button className="send" onClick={send} aria-label="Enviar mensaje">➤</button><small>Enter para enviar · Respuesta humana</small></div></section>
...
function Superadmin({notify}:{notify:(s:string)=>void}) { const [data,setData]=useState<any>(null); const [loading,setLoading]=useState(true); const load=async()=>{try{setLoading(true);setData(await api.superadminOverview())}catch(reason){notify(reason instanceof Error?reason.message:"No se pudo cargar el panel de plataforma")}finally{setLoading(false)}}; useEffect(()=>{void load()},[]); if(loading)return <EmptyState title="Cargando plataforma" note="Calculando consumo y operación real de los negocios."/>; if(!data)return <EmptyState title="Sin acceso" note="Este panel requiere la cuenta superadministradora."/>; const s=data.summary; const money=(c:number)=>new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN"}).format(c/100); return <section className="page-stack"><div className="section-heading"><div><span className="eyebrow">ACCESO PRIVADO · SUPERADMIN</span><h2>Operación de la plataforma</h2><p>Consumo real por negocio. Los costos de IA sólo se muestran si configuraste sus tarifas por millón de tokens.</p></div><button onClick={load}>Actualizar</button></div><div className="metric-grid"><article className="metric-card"><span>Negocios activos</span><h3>{s.activeOrganizations}</h3><small>{s.organizations} registrados</small></article><article className="metric-card"><span>Clientes pagando</span><h3>{s.payingOrganizations}</h3><small>{s.trials} en prueba</small></article><article className="metric-card"><span>MRR estimado</span><h3>{money(s.estimatedMrrCents)}</h3><small>Planes activos, antes de impuestos</small></article><article className="metric-card"><span>Costo IA estimado</span><h3>US$ {Number(s.estimatedAiCostUsd).toFixed(4)}</h3><small>{Number(s.aiInputTokens+s.aiOutputTokens).toLocaleString("es-MX")} tokens este mes</small></article></div><article className="card"><CardTitle title="Clientes y consumo" note="Datos operativos del periodo actual"/><div className="table-wrap"><table><thead><tr><th>Negocio</th><th>Plan</th><th>Conversaciones</th><th>IA</th><th>Canales</th><th>Costo IA</th></tr></thead><tbody>{data.clients.map((client:any)=><tr key={client.id}><td><b>{client.name}</b><small>{client.slug}</small></td><td>{client.subscription?`${client.subscription.plan} · ${client.subscription.status}`:"Sin suscripción"}</td><td>{client.usage.conversations} / {client.usage.conversationLimit||"—"}</td><td>{client.usage.aiReplies} respuestas</td><td>{client.channels.length?client.channels.join(", "):"Sin conectar"}</td><td>US$ {Number(client.usage.estimatedCostUsd).toFixed(4)}</td></tr>)}</tbody></table></div></article></section>}

function SuperadminLegacy({notify}:{notify:(s:string)=>void}) {const [provider,setProvider]=useState("Proveedor local");return <section className="page-stack"><div className="section-heading"><div><span className="eyebrow">ACCESO PRIVADO · SUPERADMIN</span><h2>Proveedores de inteligencia artificial</h2></div></div></section>}
