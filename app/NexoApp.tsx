...
function Login({ onLogin, onRegister }: { onLogin: (email:string,password:string,remember:boolean) => Promise<void>; onRegister: (data:{name:string;email:string;password:string;businessName:string;plan:BillingPlan;interval:BillingInterval}) => Promise<void> }) {
  ...
  return <div className="login-page">
    ...
    <div className="login-panel"><form onSubmit={submit} autoComplete="on">
      ...
      {mode==="register"&&<div className="form-grid compact"><label>Plan<select value={plan} onChange={event=>setPlan(event.target.value as BillingPlan)}><option value="MICRO">Micro · 20 conversaciones</option><option value="STARTER">Starter</option><option value="PRO">Growth</option><option value="ENTERPRISE">Advanced</option></select></label><label>Periodo<select value={interval} onChange={event=>setInterval(event.target.value as BillingInterval)}><option value="MONTHLY">Mensual</option><option value="YEARLY">Anual</option></select></label></div>}
